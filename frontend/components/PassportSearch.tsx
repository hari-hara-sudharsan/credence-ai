...
export default function PassportSearch() {
  ...
  const canSearch = wallet.trim().length > 0;
  ...
  return (
    ...
    >
      <input
        placeholder="Look up any wallet's passport…"
        value={wallet}
      ...
      />
      <button
        onClick={() => canSearch && router.push(`/passport/${wallet}`)}
        disabled={!canSearch}
        style={{
          ...
          border: "none",
          borderRadius: 10,
          color: !canSearch ? "#2E4060" : "#fff",
          fontSize: 14,
          fontWeight: 700,
          padding: "12px 24px",
          cursor: !canSearch ? "not-allowed" : "pointer",
          transition: "all 0.2s ease",
          boxShadow: !canSearch ? "none" : "0 4px 20px rgba(0, 229, 255, 0.2)",
          letterSpacing: 0.3,
          whiteSpace: "nowrap",
